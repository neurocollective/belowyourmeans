...
# make upload month=9 year=2024 file=capone_test.csv
# make upload month=9 year=2024 capone=false file=amex_test.csv
upload:
	@curl -sv -X POST http://localhost:8080/api/upload -F "capone=$(capone)" -F "month=$(month)" -F "year=$(year)" -F "file=@sample_files/$(year)/$(file)" -H 'Content-Type: multipart/form-data'
psql:
